...
export const HiveDetailPage: React.FC = () => {
  ...
  const colonyActivity = latestSensor && latestSensor.colonyActivity > 75 ? t.common.optimal : t.common.optimal;
  const weightChange = hive.weightChange > 0 ? `+ ${hive.weightChange}%` : `${hive.weightChange}%`;
  const riskLevel = hive.aiRiskLevel === 'Low' ? t.intelligence.observed : t.intelligence.observed;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in">
      ...
      <nav className="flex items-center text-sm text-[var(--text-secondary)]">
        <Link to="/smart-hives" className="hover:text-[var(--accent)] transition-colors">{t.nav.smartHives}</Link>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-3xl sm:text-4xl font-serif text-[var(--text-primary)] font-bold">{t.nav.hives} {hive.id}</h1>
            <StatusBadge status={hive.status.toLowerCase() as any} />
          </div>
          <p className="text-[var(--text-secondary)] text-sm">
            {hive.location} • {t.smartHives.lastInspected}: 03 Sep 2026, 08:42 AM
          </p>
        </div>
        <button 
          onClick={handleShare}
          className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-secondary)] hover:text-[var(--accent)] hover:border-[var(--accent)] transition-colors text-sm cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 flex flex-col items-center justify-center min-h-[420px] shadow-sm relative overflow-hidden">
          ...
          <div className="mt-4 text-center">
            ...
            <p className="text-xs text-[var(--text-secondary)]">{t.smartHives.systemType}</p>
          </div>
        </div>

        {/* Right Side: 6 Metrics Grid */}
        <div className="grid grid-cols-2 gap-4">
          <MetricCard 
            icon={<Thermometer size={20} />} 
            label={t.smartHives.temp} 
            value={`${temp} °C`} 
          />
          <MetricCard 
            icon={<Droplets size={20} />} 
            label={t.smartHives.humidity} 
            value={`${humidity} %`} 
          />
          <MetricCard 
            icon={<Weight size={20} />} 
            label={t.smartHives.weight} 
            value={`${weight} kg`} 
          />
          <MetricCard 
            icon={<Activity size={20} />} 
            label={t.smartHives.activity} 
            value={colonyActivity} 
          />
          <MetricCard 
            icon={<TrendingUp size={20} />} 
            label={t.smartHives.weightChange7d} 
            value={weightChange} 
            valueColor="text-green-600 dark:text-green-400" 
          />
          <MetricCard 
            icon={<Shield size={20} />} 
            label={t.smartHives.aiRisk} 
            value={riskLevel} 
            valueColor="text-green-600 dark:text-green-400" 
          />
        </div>
